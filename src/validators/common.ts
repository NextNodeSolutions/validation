 ...
 */
export const NextNodeProjectSlug = type('string').narrow((slug): slug is string => {
...
const RESERVED_USERNAMES = new Set([
  'admin', 'api', 'app', 'apps', 'auth', 'blog', 'dashboard',
  'docs', 'help', 'mail', 'root', 'support', 'system', 'test',
  'www', 'nextnode', 'dev', 'staging', 'production', 'config'
])

export const NextNodeUsername = type('string').narrow((username): username is string => {
 ...
 */
export const NextNodeSemVer = type('string').narrow((version): version is string => {
...
export const createNextNodeURL = (options: { allowHttp?: boolean; allowLocalhost?: boolean } = {}) => type('string').narrow((url): url is string => {
    try {
      ...
      const dangerousPorts = ['22', '25', '135', '139', '445', '993', '995']
