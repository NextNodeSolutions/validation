...
/**
 * NextNode Project object validator
 */
export const NextNodeProject = type({
  ...
  return name.length >= 3 && name.length <= 100 && desc.length <= 500
})
...
export const NextNodePackage = type({
  ...
  'createdAt?': 'Date',
  'updatedAt?': 'Date'
}).narrow(obj => obj.name.length >= 3 && obj.name.length <= 100 && obj.description.length <= 1000 && (obj.downloads || 0) >= 0)
...
export const NextNodeEnvironment = type({
  ...
  return !limits || ((limits.requests || 1) >= 1 && (limits.window || 1000) >= 1000)
})
...
export const NextNodeOrganization = type({
  ...
  'createdAt?': 'Date',
  'updatedAt?': 'Date'
}).narrow(obj => obj.name.length >= 2 && obj.name.length <= 100 && (!obj.description || obj.description.length <= 500))
...
export const NextNodeWebhook = type({
  ...
  return retryCount >= 0 && retryCount <= 5 && lastStatus >= 100 && lastStatus <= 599
})
