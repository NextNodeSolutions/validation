	...
	describe('Conditional Validation', () => {
		describe('validateIf', () => {
			it('should validate when condition is met', () => {
				const validator = validateIf(
					(data: unknown) =>
						(data as Record<string, unknown>).type === 'premium',
					() => 'valid_result', // Mock validator
					'subscription',
				)
			...
			it('should skip validation when condition is not met', () => {
				const validator = validateIf(
					(data: unknown) =>
						(data as Record<string, unknown>).type === 'premium',
					() => ({ message: 'error', summary: 'validation failed' }), // Mock validator that would fail
					'subscription',
				)
		...
		describe('requireAtLeastOne', () => {
			it('should pass when at least one field is present', () => {
				...
				const data = { email: 'test@example.com' } as Record<
					string,
					unknown
				>
			...
			it('should fail when no required fields are present', () => {
				const validator = requireAtLeastOne('email', 'phone')
				const data = { name: 'John' } as Record<string, unknown>
		...
		describe('requireExactlyOne', () => {
			it('should pass when exactly one field is present', () => {
				...
				const data = { password: 'secret123' } as Record<
					string,
					unknown
				>
		...
		describe('requireWhen', () => {
			...
			it('should fail when dependent field is present but required field is missing', () => {
				const validator = requireWhen('hasAddress', 'address')
				const data = { hasAddress: true } as Record<string, unknown>
			...
			describe('validateProjectSettings', () => {
				it('should validate public project settings', () => {
					const data = {
						visibility: 'public',
						description:
							'This is a public project with good description',
	...
	describe('Async Validation', () => {
		describe('withCache', () => {
			it('should cache validation results', async () => {
				...
				const cachedValidator = withCache(mockValidator, {
					cacheTtl: 1000,
				})
		...
		describe('combineAsyncValidators', () => {
			it('should combine multiple async validators', async () => {
				const validator1 = vi.fn(async (value: string) => ({
					success: true as const,
					data: value,
				}))
				const validator2 = vi.fn(async (value: string) => ({
					success: true as const,
					data: value,
				}))

				const combinedValidator = combineAsyncValidators([
					validator1,
					validator2,
				])
			...
			it('should fail if any validator fails', async () => {
				...
				const validator2 = vi.fn(async () => ({
					...
					errors: [
						{ key: 'INVALID_FORMAT' as const, reason: 'Failed' },
					],
				}))
	...
	describe('Transformations', () => {
		describe('StringTransforms', () => {
			it('should normalize strings', () => {
				expect(StringTransforms.normalize('  HELLO WORLD  ')).toBe(
					'hello world',
				)
			})
			...
			it('should slugify strings', () => {
				expect(StringTransforms.slugify('Hello World! @#$')).toBe(
					'hello-world',
				)
			})

			it('should convert to title case', () => {
				expect(StringTransforms.titleCase('hello world')).toBe(
					'Hello World',
				)
			})
			...
			it('should collapse whitespace', () => {
				expect(
					StringTransforms.collapseWhitespace('hello    world'),
				).toBe('hello world')
			})

			it('should strip HTML', () => {
				expect(
					StringTransforms.stripHTML('<p>Hello <b>World</b></p>'),
				).toBe('Hello World')
			})
		})
		...
		describe('EmailTransforms', () => {
			it('should normalize Gmail addresses', () => {
				expect(
					EmailTransforms.normalize('test.email+tag@gmail.com'),
				).toBe('testemail@gmail.com')
			})

			it('should handle non-Gmail addresses', () => {
				expect(
					EmailTransforms.normalize('test.email+tag@company.com'),
				).toBe('test.email@company.com')
			})

			it('should simple normalize emails', () => {
				expect(EmailTransforms.simple('  TEST@COMPANY.COM  ')).toBe(
					'test@company.com',
				)
			})
		})
		...
		describe('NextNodeTransforms', () => {
			it('should transform usernames', () => {
				expect(NextNodeTransforms.username('John Doe!')).toBe(
					'john_doe',
				)
				expect(NextNodeTransforms.username('___test___')).toBe('test')
				expect(
					NextNodeTransforms.username('a'.repeat(40)),
				).toHaveLength(30) // max length
			})

			it('should transform project slugs', () => {
				expect(
					NextNodeTransforms.projectSlug('My Awesome Project!'),
				).toBe('my-awesome-project')
				expect(NextNodeTransforms.projectSlug('test__project')).toBe(
					'test-project',
				)
			})
			...
			it('should normalize organization names', () => {
				expect(
					NextNodeTransforms.organizationName(
						'NextNode   Solutions & Co.',
					),
				).toBe('NextNode Solutions & Co.')
			})

			it('should clean API key names', () => {
				expect(
					NextNodeTransforms.apiKeyName('Production API Key!'),
		...
		describe('transformThenValidate', () => {
			it('should transform then validate successfully', () => {
				...
				const pipeline = transformThenValidate(
					(s: string) => s.trim(),
					mockValidator,
				)
