 ...
 */
export const creditCard = type('string').narrow((cardNumber, ctx) => {
 ...
 */
export const currencyCode = type(/^[A-Z]{3}$/)

/**
 * Price/amount: positive number with max 2 decimal places
 */
export const price = type('number > 0').narrow((n, ctx) => {
 ...
 */
export const iban = type(/^[A-Z]{2}[0-9]{2}[A-Z0-9]{4,30}$/)

/**
 * BIC/SWIFT code
 */
export const bic = type(/^[A-Z]{4}[A-Z]{2}[A-Z0-9]{2}([A-Z0-9]{3})?$/)

/**
 * Amount with optional negative (for transactions)
 */
export const amount = type('number').narrow((n, ctx) => {
 ...
 */
export const taxIdUS = type(/^\d{2}-\d{7}$/) // EIN format
export const taxIdFR = type(/^FR\d{11}$/) // French VAT
