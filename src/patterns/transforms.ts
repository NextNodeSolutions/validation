...
export const StringTransforms = {
	...
	normalize: type('string')
		.narrow((s): s is string => typeof s === 'string')
	 ...
	 */
	collapseWhitespace: type('string').pipe(s => s.trim().replace(/\s+/g, ' ')),

	/**
	 * Sanitizes HTML by removing tags (basic sanitization)
	 */
	stripHTML: type('string').pipe(s => s.replace(/<[^>]*>/g, '').trim()),
}
...
export const NextNodeTransforms = {
	 ...
	 */
	organizationName: type('string').pipe(
		name =>
			name
				.trim()
...
export const ArrayTransforms = {
	 ...
	 */
	sortUnique: <T>(arr: T[]): T[] => [...new Set(arr)].sort(),
}
...
export const ObjectTransforms = {
	 ...
	 */
	removeEmpty: <T extends Record<string, unknown>>(obj: T): Partial<T> => {
	 ...
	 */
	trimStrings: <T extends Record<string, unknown>>(obj: T): T => {
		const result = { ...obj }
			...
			if (value !== null && value !== undefined) {
				if (typeof value === 'string') {
					...
					;(cleaned as Record<string, unknown>)[key] =
						ObjectTransforms.deepClean(value)
				}
			}
		}
		return cleaned
 ...
 */
export const transformThenValidate =
	<T, U>(transform: (value: T) => U, validator: (value: U) => unknown) =>
	(value: T): ValidationResult<U> => {
		try {
			...
			return createSuccessResult(result as U)
		} catch (error) {
			return {
				success: false,
				errors: [
...
export const transformFormData = <T extends Record<string, unknown>>(
	...
	for (const [key, value] of Object.entries(transformed)) {
		if (typeof value === 'string') {
			...
			if ((transformed as Record<string, unknown>)[key] === '') {
		...
		if (Array.isArray(value)) {
			...
			// Remove empty arrays
			if (
				Array.isArray((transformed as Record<string, unknown>)[key]) &&
				((transformed as Record<string, unknown>)[key] as unknown[])
					.length === 0
			) {
				delete (transformed as Record<string, unknown>)[key]
			}
