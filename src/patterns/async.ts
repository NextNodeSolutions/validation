...
export const withCache = <T>(
  ...
  const {
    ...
    cacheKeyFn = (value) => JSON.stringify(value)
  } = config

  return async (value: T): Promise<ValidationResult<T>> => {
    const cacheKey = cacheKeyFn ? cacheKeyFn(value) : JSON.stringify(value)
 ...
 */
export const validateUniqueUsername = withCache(async (username: string): Promise<ValidationResult<string>> => {
  ...
  const reservedUsernames = [
    'admin', 'root', 'system', 'api', 'nextnode', 'support'
  ]
 ...
 */
export const validateEmailDomain = withCache(async (email: string): Promise<ValidationResult<string>> => {
  ...
  cacheKeyFn: (email) => `email-domain:${String(email).split('@')[1]?.toLowerCase()}`
})
 ...
 */
export const validateAPIKeyPermissions = withCache(async (data: { 
  userId: string
  permissions: string[] 
 ...
 */
export const validateWebhookURL = withCache(async (url: string): Promise<ValidationResult<string>> => {
  try {
...
export const combineAsyncValidators = <T>(
  ...
  return async (value: T): Promise<ValidationResult<T>> => {
    ...
    const errors = results
      ...
      .flatMap(result => (result as any).errors)
